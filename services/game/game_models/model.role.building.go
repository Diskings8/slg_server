...
type RoleBuilding struct {
	models.ModelBase
	RoleID    uint64                    `gorm:"column:role_id;type:bigint(20);not null;index:idx_role_building"`
	...
	Level     uint32                    `gorm:"column:level;type:int(11) unsigned;not null;default:0"` // 当前生效等级（建造中=0）
	State     pb_city.BuildingState     `gorm:"column:state;type:int(11);not null;default:0"` // 建筑状态
	CityID    uint64                    `gorm:"column:city_id;type:bigint(20);not null;default:0"` // 归属城市（校场/兵营等附属建筑；主城为 0）
	NextLevel uint32                    `gorm:"column:next_level;type:int(11) unsigned;not null;default:0"` // 建造/升级目标等级（Completed 为 0）
	EndTimeUx int64                     `gorm:"column:end_time_ux;type:bigint(20);not null;default:0"` // 完成时间戳（Completed 为 0）
}
