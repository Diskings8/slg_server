...
| 维度 | ldl | ai4slg (07-28) | ai4slg (07-30) | ai4slg (08-04) | ai4slg (08-05) | 进展 |
|------|-----|----------------|----------------|----------------|----------------|------|
| Go 源文件数 | 1251 | 40 | 47 | **85** | **109** | +69 |
| 实体子模块 | 24 个 | 4 个 | 5 个 | **7 个** (+Buildings/Formations) | **7 个** | +3 🟢 |
| 内部业务逻辑模块 | 50+ | 0 (逻辑在 cores 层) | 2 个 | **8 个** (hero/troop/skill/star/item/formation/building/march) | **9 个** (+exchange) | +9 🟢 |
| Stream 协议处理器 | ~44 个 | 0 (空骨架) | 2 个 | **20 个** | **21 个** | +21 🟢 |
| RPC Unary 处理器 | 16 个 | 2 个 (仅日志) | 2 个 (统一 Do) | 2 个 (统一 Do) | 2 个 (统一 Do) | 🟡 |
| 架构层次 | 6 层完整 | 3 层雏形 | 4 层 | **4 层** (handler→logic→entity→model) | **4 层** | 🟢 |
| 测试文件 | 大量 | 1 个 | 1 个 | **4 个** (role/currency/skill/star) | **5 个** (+exchange) | 🟢 |
...
| 货币兑换 | — | **`CurrencyExchange()`** 一级→二级，配置比例整组兑换 | 🆕 |
...
| 1000035 | GameCurrencyExchange | CurrencyExchange | item_handler |
...
12. ✅ **货币兑换** - 一级→二级，exchange 规则表驱动 + 整组兑换 + 产消日志 reason="exchange"
...
| **2.8** | **货币兑换** — 一级→二级 | ✅ 配置规则表驱动(1 钻石=10 金币) + 整组兑换 + 产消日志 reason="exchange"（08-05） |
...
### 👉 下一步建议: Phase 3 剩余子系统 / Phase 4 战斗打通

| 内容 | 理由 |
|------|------|
| **Phase 4 战斗打通** | 战斗框架已建、属性已接入战斗、经验结算+战报已通；可继续打通"战斗→占城/驻军/攻防"完整闭环 |
| **Phase 3 剩余子系统** | Equips 装备 / Tasks 任务 / 商店 / 联盟等仍缺失；配置表系统就绪后，新子系统可直接以 JSON 表接入 |
| 备选 | **2.6 英雄治疗**（锁已实现，治疗未做，Phase 2 唯一缺口） |
